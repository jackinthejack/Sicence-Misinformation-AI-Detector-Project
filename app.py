...
def calibration_chart(title: str, accuracy, confidence):
...
def generate_ai_summary(metrics):
    ...
    prompt = f"""
You are analyzing the results of a middle school AI science experiment.

Quick Mode vs Reasoning Mode results:

Accuracy Quick: {metrics['acc_q']:.1f}%
Accuracy Reasoning: {metrics['acc_r']:.1f}%

Consistency Quick: {metrics['cons_q']:.1f}%
Consistency Reasoning: {metrics['cons_r']:.1f}%

Average Confidence Quick: {metrics['conf_q']:.1f}
Average Confidence Reasoning: {metrics['conf_r']:.1f}

Uncertainty Rate Quick: {metrics['unc_q']:.1f}%
Uncertainty Rate Reasoning: {metrics['unc_r']:.1f}%

Responsible Answer Score Quick: {metrics['ras_q']:.1f}
Responsible Answer Score Reasoning: {metrics['ras_r']:.1f}

Overconfidence Rate Quick: {metrics['over_q']:.1f}%
Overconfidence Rate Reasoning: {metrics['over_r']:.1f}%

Write a short scientific interpretation explaining:

• what the results suggest  
• whether reasoning improved reliability  
• why uncertainty can be scientifically responsible  
• a clear takeaway judges can understand
"""

    r = client.chat.completions.create(
        model=MODEL,
        messages=[
            {"role":"system","content":"You analyze scientific experiment results."},
...
# ---------------------------------------------------------
    ...
    with c2:
        # Consistency is only meaningful when you use 3-runs, but we show it anyway.
...
st.write(
"""
This chart compares **how confident the AI is** with **how often it is actually correct**.

If confidence is much higher than accuracy, the AI may be **overconfident**.

If confidence and accuracy are close, the AI is **well calibrated**, meaning it understands when it might be wrong.
"""
)
